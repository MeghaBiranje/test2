 ...
 * @since  1.6
 */
class BusinessModelEmployeeForm extends \Joomla\CMS\MVC\Model\FormModel
{
    private $item = null;
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_business.employee', 'employeeform', array(
                        'control'   => 'jform',
                        'load_data' => $loadData
                )
        );
    ...
    {
        $data = Factory::getApplication()->getUserState('com_business.edit.employee.data', array());
    ...
    public function delete($pk)
    {
        $user = Factory::getUser();
            ...
            if (empty($pk))
            {
                    $pk = (int) $this->getState('employee.id');
            }
